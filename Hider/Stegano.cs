...
namespace Hider
{
    internal class Stegano
    {
        ...
        public static List<Bitmap> HideDataToFrames(
            Hider hider, 
            List<Bitmap> frames, 
            byte[] data, 
            int width, 
            ...
            int level = 1)
        {
            byte[] signal = new byte[] { 0xd, 0xe, 0xf, (byte)level }; 

            Color pixel, newColor;

            uint hash32 = CRC32(data);

            byte[] size = BitConverter.GetBytes(data.Length);
            byte[] hash = BitConverter.GetBytes(hash32);

            byte[] content = new byte[data.Length + signal.Length + hash.Length + size.Length];

            hider.SetProgessbar(content.Length);

            signal.CopyTo(content, 0);
            hash.CopyTo(content, 0 + signal.Length);
            size.CopyTo(content, 0 + signal.Length + hash.Length);
            data.CopyTo(content, 0 + signal.Length + hash.Length + size.Length);

            if (content.Length * 8 > (long)frames.Count * (long)width * (long)height * (long)3 * (long)level)
            {
            ...
            for (int lv = 1; lv <= level; ++lv)
            {
                ...
                for (int frameCounter = 0; frameCounter < frames.Count; ++frameCounter)
                {
                    for (int x = 0; x < width; ++x)
                    {
                        ...
                        {
                            pixel = frames[frameCounter].GetPixel(x, y);
                            int r = pixel.R;
                            int g = pixel.G;
                            int b = pixel.B;
                            ...
                            {
                                r -= r & delta;
                                r += ((content[byteCounter] >> bitCounter) & 1);  // (((content[byteCounter] >> bitCounter) & 1)) << (level-1)
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                                if (byteCounter >= content.Length)
                                {
                                    break;
                                }

                                g -= g & delta;
                                g += ((content[byteCounter] >> bitCounter) & 1);
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                                if (byteCounter >= content.Length)
                                {
                                ...
                                b -= b & delta;
                                b += ((content[byteCounter] >> bitCounter) & 1);
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                            ...
                            frames[frameCounter].SetPixel(x, y, newColor);
                            if (byteCounter >= content.Length)
                            {
                                break;
                            }
                        }
                        hider.UpdateProgessbar(byteCounter);
        ...
        public static byte[] GetDataFromFrames(Hider hider, List<Bitmap> frames, int width, int height)
        {
            Color pixel;
            
            int headerSize = 12;
            byte[] content = new byte[headerSize];

            int byteCounter = 0;
            int level = 4;
            uint signal = 0;
            uint hash = 0;
            uint contentLength = 13;
                ...
                for (int frameCounter = 0; frameCounter < frames.Count; ++frameCounter)
                {
                    for (int x = 0; x < width; ++x)
                    {
                        for (int y = 0; y < height; ++y)
                        {
                            ...
                            for (int _ = 0; _ < 1; ++_)
                            {
                                content[byteCounter] += (byte)((r & delta) << bitCounter);
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                                ...
                                content[byteCounter] += (byte)((g & delta) << bitCounter);
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                                ...
                                content[byteCounter] += (byte)((b & delta) << bitCounter);
                                byteCounter += (++bitCounter) >> 3;
                                bitCounter &= 7;
                            ...
                            if (false == headerSuccess && headerSize == byteCounter)
                            {
                                headerSuccess = true;
                                signal = BitConverter.ToUInt32(content, 0);

                                if ((signal & 0xfffff) != 0x0f0e0d)
                                {
                                    throw new Exception("No hidden data");
                                }
                                level = (int)(signal >> 24);
                                hash = BitConverter.ToUInt32(content, 4);
                                contentLength = BitConverter.ToUInt32(content, 8);
                                content = new byte[contentLength];
                                byteCounter = 0; 
                                bitCounter = 0;

                                hider.SetProgessbar((int)contentLength);
                            }
            ...
            uint newHash = CRC32(content);
            if (newHash != hash) 
            {
                throw new Exception("Extract data failed. Original hash: " + hash.ToString() + ". Current hash: " + newHash.ToString());
            }
            return content;
        }
...
//class Stegano
//{
//    // Hàm hash CRC32 trả về giá trị hash CRC32 của dữ liệu
//    static uint CRC32(byte[] s);

//    // Hàm HideTextToFrames trả về danh sách các frame của video đã được thêm dữ liệu
//    public static List<Bitmap> HideTextToFrames(
//        Hider hider,            // Windows form: dùng để cập nhật progessbar
//        List<Bitmap> frames,    // Danh sách các frame của Video
//        byte[] data,            // Dữ liệu cần ẩn giấu
//        int width,              // Chiều rộng của khung hình
//        int height,             // Chiều cao của khung hình
//        int level = 1           // số bit thấp nhất sử dụng để thêm dữ liệu
//    );

//    // Hàm UncoverMessageFromFrames trả về dữ liệu được ẩn giấu trong video
//    public static byte[] UncoverMessageFromFrames(
//        Hider hider,            // Windows form: dùng để cập nhật progessbar
//        List<Bitmap> frames,    // Danh sách các frame của Video
//        int width,              // Chiều rộng của khung hình
//        int height              // Chiều cao của khung hình
