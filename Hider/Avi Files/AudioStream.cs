...
namespace AviFile
{
	public class AudioStream : AviStream{
		...
		public AudioStream(IntPtr aviFile, IntPtr aviStream){
		...
		public IntPtr GetStreamData(ref Avi.AVISTREAMINFO streamInfo, ref Avi.PCMWAVEFORMAT format, ref int streamLength){
			...
			streamLength = Avi.AVIStreamLength(aviStream) * streamInfo.dwSampleSize;
			IntPtr waveData = Marshal.AllocHGlobal(streamLength);
			
			int result = Avi.AVIStreamRead(aviStream, 0, streamLength, waveData, streamLength, IntPtr.Zero, IntPtr.Zero);
			if(result != 0){
