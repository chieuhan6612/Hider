...
namespace AviFile
{
	public class AviManager
	{
		...
		public AviManager(String fileName, bool open){
			...
			if(open){ //open existing file
				...
				result = Avi.AVIFileOpen(
					...
					Avi.OF_READWRITE, IntPtr.Zero);

			}else{ //create empty file
				
				result = Avi.AVIFileOpen(
					ref aviFile, fileName, 
					Avi.OF_WRITE | Avi.OF_CREATE, IntPtr.Zero);
        ...
        private AviManager(IntPtr aviFile) {
        ...
        public void AddAudioStream(AudioStream newStream, int startAtFrameIndex) {
			...
			result = Avi.AVIStreamWrite(aviStream, 0, streamLength, waveData, streamLength, Avi.AVIIF_KEYFRAME, IntPtr.Zero, IntPtr.Zero);
			if(result != 0){
