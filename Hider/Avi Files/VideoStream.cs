...
namespace AviFile
{
	public class VideoStream : AviStream
	{
        ...
        public VideoStream(IntPtr aviFile, bool writeCompressed, double frameRate, int frameSize, int width, int height, PixelFormat format) {
        ...
        public VideoStream(IntPtr aviFile, bool writeCompressed, double frameRate, Bitmap firstFrame) {
        ...
        /// <param name="frameRate">Frames per second</param>
        /// <param name="firstFrame">Image to write into the stream as the first frame</param>
        public VideoStream(IntPtr aviFile, Avi.AVICOMPRESSOPTIONS compressOptions, double frameRate, Bitmap firstFrame) {
		...
		public VideoStream(IntPtr aviFile, IntPtr aviStream){
			this.aviFile = aviFile;
			...
			this.firstFrame = Avi.AVIStreamStart(aviStream);
			this.countFrames = Avi.AVIStreamLength(aviStream);
		}
        ...
        /// <param name="frameRate">Frames per second</param>
        /// <param name="firstFrame">Image to write into the stream as the first frame</param>
        private void Initialize(IntPtr aviFile, bool writeCompressed, double frameRate, Bitmap firstFrameBitmap) {
            this.aviFile = aviFile;
            this.writeCompressed = writeCompressed;
            this.frameRate = frameRate;
			this.firstFrame = 0;
		...
		private void CreateCompressedStream(){
            ...
            int result = Avi.AVIMakeCompressedStream(out compressedStream, aviStream, ref compressOptions, IntPtr.Zero);
            if(result != 0) {
        ...
        private void CreateCompressedStream(Avi.AVICOMPRESSOPTIONS options) {
            int result = Avi.AVIMakeCompressedStream(out compressedStream, aviStream, ref options, IntPtr.Zero);
            if (result != 0) {
                throw new Exception("Exception in AVIMakeCompressedStream: " + result.ToString());
            }
		...
		public void GetFrameOpen(){
            ...
            if(getFrameObject == IntPtr.Zero){ throw new Exception("Exception in VideoStreamGetFrameOpen!"); }
		}
		...
		public Bitmap GetBitmap(int position){
            ...
            IntPtr dib = Avi.AVIStreamGetFrame(getFrameObject, firstFrame + position);

            //Copy the bitmap header into a managed struct
            bih.bmiColors = this.palette;
            bih.bmiHeader = (Avi.BITMAPINFOHEADER)Marshal.PtrToStructure(dib, bih.bmiHeader.GetType());
            ...
            IntPtr dibPointer = new IntPtr(dib.ToInt64() + Marshal.SizeOf(bih.bmiHeader) + framePaletteSize);
